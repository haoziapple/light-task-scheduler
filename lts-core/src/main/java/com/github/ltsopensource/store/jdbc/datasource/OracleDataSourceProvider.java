...
public class OracleDataSourceProvider implements DataSourceProvider {
    ...
    public DataSource getDataSource(Config config) {
        ...
        LOGGER.info("Getting DataSource, url:" + url + " ,username:" + username + " ,password:" + password);
        ...
        if (dataSource == null) {
            try {
                synchronized (lock) {
                    ...
                    LOGGER.info("Creating Druid DataSource, cachedKey:" + cachedKey);
...
//        FIELDS.put("driverClassName", String.class);
        System.out.println("无需设置driverClassName, DruidDataSource会根据连接url自动选择driverClassName");
